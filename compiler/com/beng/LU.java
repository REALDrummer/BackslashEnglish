...
public class LU {
	 ...
	 * 
	 * @param strings is the list of <tt>String</tt>s that will be combined into a single <tt>String</tt>.
	 * @param separator is the String used to separate the different <tt>String</tt>s, e.g. ", " in the list
	 *                "apple, orange, lemon, melon"
	 * @param indices is an optional parameter that can be used to select a range of indices in the array <b>
	 *                <tt>strings</b></tt>. If one index is given, it will be used as the minimum index (inclusive)
	 *                for parsing <b><tt>strings</b></tt> for adding pieces to the resultant <tt>String</tt>. If two
	 *                indices are given, the first index is used as the minimum index (inclusive) and the second is
	 *                used as the maximum (non-inclusive).
	 * @return the <tt>String</tt> constructed by putting all the <tt>String</tt>s in <b><tt>strings</tt></b>
	 *         together into one <tt>String</tt>. */
	public static String combine(String[] strings, String separator, int... indices) {
		...
		for (int i = start_index; i < end_index; i++) {
			try {
				...
				// before throwing the exception, write "indices" into an Integer[] so it can be passed
				// as an Object[] into writeArray()
				...
				throw new RuntimeException("Someone gave combine() bad indices!\nindices: "
						+ writeArray(integer_indices), exception);
			}
			if (i < end_index - 1)
	...
	/** This method compares two lists of Objects, giving the greatest priority to the first element in the lists.
	 * In other words, it compares the first element in the <b> <tt>initial_array</b></tt> to the first element in
	 * the <b><tt>compare_array</b></tt> (using {@link Comparable#compareTo(Object)} if available, comparing
	 * <tt>toString()</tt> outputs otherwise). If the comparison results in something other than a 0, it returns
	 * that value; otherwise, it compares the next element in each array until it gets a non-zero result. If the end
	 * of one or both lists is reached, 0 is returned.
	 * <hr>
	 * If an element in either array is <b>null</b>, the comparison automatically results in a 0; in other words,
	 * <b>null</b> can be used as a "wild card".
	 * 
	 * @param initial_array is the array that will be compared to <b><tt>compare_array</b></tt>. If <b>
	 *                <tt>initial_array</b></tt> > <b><tt>compare_array</b></tt>, this method will return a
	 *                <i>positive</i> integer.
	 * @param compare_array is the array that will be compared to <b><tt>initial_array</b></tt>. If <b>
	 *                <tt>compare_array</b></tt> > <b><tt>initial_array</b></tt>, this method will return a
	 *                <i>negative</i> integer.
	 * @return the first comparison of elements in the arrays that returns something besides 0 or 0 if all elements
	 *         are equivalent. */
	@SafeVarargs
	public static <T> int compare(T[] initial_array, T... compare_array) {
		...
		for (int i = 0; i < initial_array.length && i < compare_array.length; i++) {
			...
			if (initial_array[i] == null || compare_array[i] == null)
				...
				try {
					compare = (int) initial_array[i].getClass()
							.getMethod("compareTo", initial_array[i].getClass())
							.invoke(compare_array[i]);
	...
	/** This method copies all the <tt>Objects</tt> in all the given <tt>Object[]</tt>s into one large array in
	 * which the elements are ordered according to the order of the arrays given as arguments.
	 * 
	 * @param arrays is the list of <tt>Object[]</tt>s to be concatenated into one large array.
	 * @return one large <tt>Object[]</tt> containing all the elements of the given <b><tt>arrays</b></tt> in the
	 *         order given. */
	@SafeVarargs
	 ...
	 * 
	 * @param ints is the array of ints that will be searched.
	 * @param target is the int that <b><tt>ints</b></tt> may contain.
	 ...
	 * 
	 * @param shorts is the array of shorts that will be searched.
	 * @param target is the short that <b><tt>shorts</b></tt> may contain.
	 * @return <b>true</b> if <b><tt>object</b></tt> contains <b><tt>target</b></tt>; <b>false</b> otherwise. */
	 ...
	 * 
	 * @param objects is the array of Objects that will be searched.
	 * @param target is the Object that <b><tt>objects</b></tt> may contain.
	 * @return <b>true</b> if <b><tt>object</b></tt> contains <b><tt>target</b></tt>; <b>false</b> otherwise. */
	@SafeVarargs
	 ...
	 * 
	 * @param list is the String which will be divided into separate Strings by deconstructing the list framework.
	 * @param options are optional parameters used to change the separator String and the final conjuction String.
	 *                (By default, these are ", " and "and", respectively.) The first item is the separator String
	 *                (the String used to separate the items in the list); the second item is a final conjunction
	 *                String, a String which may be attached to the beginning of the last item in the list.
		...
		if (list.contains(separator)) {
			...
			if (objects[objects.length - 1].startsWith(final_conjunction + " "))
				objects[objects.length - 1] = objects[objects.length - 1]
						.substring(final_conjunction.length() + 1);
		}
		// for 2-item lists
	...
	public static <T> T[] subArray(T[] objects, int start, int end) {
		try {
			...
			throw new RuntimeException("Someone gave subArray() bad indices!\narray=" + objects + "\nstart="
					+ start + "\nend=" + end, exception);
		}
	}
	 ...
	 * 
	 * @param objects is the String array which will be written into a list.
	 * @param separator is the String that will be used to separate terms in the list; by default, the separator is
	 *                ", "
	 * @param final_conjunction is the String that will be added between the second to last and last terms in a list
	 *                of two or more items; by default, the final conjunction is "and"
	 * @param prefix is the String that will be added to the beginning of each item in the list; by default, the
	 *                prefix is an empty String ("")
	 * @param suffix is the String that will be added to the end of each item in the list; by default, the suffix is
	 *                an empty String ("")
	...
	public static String writeMap(Map<?, ?> objects) {
		...
		int i = 0;
		for (Object key : objects.keySet()) { // this must be a for each loop with a counter rather than a
							// regular for loop because sets are not indexed
